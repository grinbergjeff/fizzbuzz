...
/* APPROACH TO SOLVE */
/* Prioritize the if statements so that the first if statement
is the divisible by 3 AND divisible by 5. This is because 
if I started with just divisible by 3, it'll print for that and
then it'll also print AGAIN on top. For example, if my code has to
check if it is divisible by 3, it will then check if it is divisible by 5 and
finally divisible by both....the program when it reaches variable=15, will have printed Fizz, Buzz AND FizzBuzz for variable=15. This is
because the variable satisfied the order of priority of if
...
checks for being divisible by both, it'll print Fizzbuzz. It
will then skip the other conditions to check for due to the
